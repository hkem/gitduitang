            ...
            <div class="recommend-list-li" v-for="item in list" :key="item.id">
              ...
              <div class="recommend-centre-block" @click="clickcentre(item.id,item.type)">
                ...
                <div class="recommend-centre-img" v-if="item.type == 2">
                  <van-image class="centreall-li-img" fit="cover" :src="item.image"/>
                </div>
  ...
  export default {
    ...
    methods:{
      ...
      get_list(p,type){
        ...
        // }
        this.$axios
             .post(this.$myStore.url+"/api/index/get_article",this.$qs.stringify({p:p,token:token,type:0,id:-1}))
             .then(response=>{
...
<style>
  ...
  .recommend-centre-img .centreall-li-img{
    width: 680px;
    height: 300px;
  }
