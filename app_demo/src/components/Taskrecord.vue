        ...
        <div class="taskrecord-list-li" v-for="item in list">
          <div class="taskrecord-li-centren">
            <div class="taskrecord-li-left">
              <div class="taskrecord-li-title">{{item.centre}}</div>
              <div class="taskrecord-li-time">{{item.ctime}}</div>
            </div>
            <div class="taskrecord-li-right" v-if="item.type == 1">+{{item.munber}}</div>
            <div class="taskrecord-li-right" v-if="item.type == 3">+{{item.munber}}</div>
            <div class="taskrecord-li-right" v-if="item.type == 2">-{{item.munber}}</div>
          </div>
        </div>
      </div>
      </van-pull-refresh>
  ...
  export default {
    ...
    methods:{
      ...
      get_list(p,type){
        ...
        this.$axios
             .post(this.$myStore.url+"/api/index/getsign_list",this.$qs.stringify({id:that.id,token:token,p:p}))
             .then(response=>{
                //成功
